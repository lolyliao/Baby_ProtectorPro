...
//	增加天(long)
//
+ (NSDate *)dateByAddingDaywithlong:(long)day toDate:(NSDate *)date
{
    NSDateComponents *components = [[NSDateComponents alloc] init];
    components.day=day;
...
+ (NSDate *)tomorrowDate
{
    return  [self dateByAddingDay:1 toDate:[NSDate date]];
}
...
- (NSString *)stringWithHumanReadEasy1
{
    NSTimeInterval interval = [self timeIntervalSinceNow];
    if (interval > 0) {
        return [self stringWithFormat:@"yyyy-MM-dd HH:mm"];
...
+ (NSString*)getWeekDisplayString2:(int)weekIndex
{
...
- (int)daydd:(NSString *)dataString
{
    ...
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    NSDate *fromDate = [dateFormatter dateFromString:dataString];
...
{
    NSDate *localeDate = [[NSDate date] toLocalTime];
    //NSLog(@"当地时间%@",localeDate);
    long difference =fabs([localeDate timeIntervalSinceDate:date]);
    return difference;
}

+ (long)compareDate:(NSDate *)date andDate2:(NSDate *)date2
{
    NSDate *localeDate = [[NSDate date] toLocalTime];
    //NSLog(@"当地时间%@",localeDate);
    long difference =fabs([localeDate timeIntervalSinceDate:date]);
...
+ (int)compareDated:(NSDate *)date01 withDate:(NSDate *)date02
{
    int ci;
    //    NSDateFormatter *df = [[NSDateFormatter alloc] init];
    //    [df setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
+ (NSDate *)to_yyyyMMdd:(NSString *)string {
    
    return [self dateFromString:string withFormat:@"yyyy-MM-dd"];
}

+ (NSDate *)to_yyyyMMddHHmmss:(NSString *)string {
